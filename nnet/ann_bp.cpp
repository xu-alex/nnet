	...
	numNodesHiddenLayer(nNHL), studyRate(_sR)
{
	...
	//创建权值空间,并初始化
	srand(time(NULL));    // 以时间为内核设计一个随机数种子
	weights = new double**[2];  // 定义权值矩阵
	weights[0] = new double *[numNodesInputLayer]; // 输入层的权值矩阵
	...
	weights[1] = new double *[numNodesHiddenLayer];// 隐藏层的权值矩阵
	...
	//创建偏置空间，并初始化
	bias = new double *[2];
	bias[0] = new double[numNodesHiddenLayer];   //隐藏层的偏置值
	for (int i = 0; i < numNodesHiddenLayer; ++i) 
	{
		bias[0][i] = (rand() % (2000) / 1000.0 - 1); //-1到1之间
	}
	bias[1] = new double[numNodesOutputLayer];  // 输出层的偏置值
